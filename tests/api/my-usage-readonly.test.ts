...
describe("my-usage API：只读 Key 自助查询", () => {
  ...
  test("今日统计：应与 message_request 数据一致，并排除 warmup 与其他 Key 数据", async () => {
    ...
    expect(breakdownByModel.get("gpt-4.1-original")?.billingModel).toBe("gpt-4.1-original");
    expect(breakdownByModel.get("gpt-4.1-mini-original")?.calls).toBe(1);
    expect(breakdownByModel.get("gpt-4.1-mini-original")?.billingModel).toBe(
      "gpt-4.1-mini-original"
    );
    ...
    const logRows = (logs.json as any).data.logs as Array<{
      id: number;
      model: string | null;
      billingModel: string | null;
  ...
  test("getMyStatsSummary：基础聚合统计，排除 warmup，区分 key/user breakdown", async () => {
    ...
    const a1 = await createMessage({
      ...
      key: keyA.key,
      model: "claude-3-opus",
      originalModel: "claude-3-opus-original",
      endpoint: "/v1/messages",
      costUsd: "0.1000",
      inputTokens: 500,
      outputTokens: 200,
    ...
    const a2 = await createMessage({
      ...
      originalModel: "claude-3-sonnet-original",
      endpoint: "/v1/messages",
      costUsd: "0.0500",
      inputTokens: 300,
      outputTokens: 100,
    ...
    expect(keyBreakdownMap.get("claude-3-opus-original")?.requests).toBe(1);
    expect(keyBreakdownMap.get("claude-3-opus-original")?.cost).toBeCloseTo(0.1, 4);
    expect(keyBreakdownMap.get("claude-3-sonnet-original")?.requests).toBe(1);
    expect(keyBreakdownMap.get("claude-3-sonnet-original")?.cost).toBeCloseTo(0.05, 4);
    ...
    expect(userBreakdownMap.get("claude-3-opus-original")?.requests).toBe(2);
    expect(userBreakdownMap.get("claude-3-opus-original")?.cost).toBeCloseTo(0.18, 4);
    // claude-3-sonnet: a2 only
    expect(userBreakdownMap.get("claude-3-sonnet-original")?.requests).toBe(1);
    // 其他用户的模型不应出现
