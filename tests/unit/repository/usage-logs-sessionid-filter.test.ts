...
describe("Usage logs sessionId filter", () => {
  ...
  test("findUsageLogsBatch: model 应按 originalModel 优先语义过滤", async () => {
    vi.resetModules();
    ...
    const { findUsageLogsBatch } = await import("@/repository/usage-logs");
    await findUsageLogsBatch({ model: "claude-sonnet-4" });

    expect(whereArgs).toHaveLength(2);
    const primaryWhereSql = sqlToString(whereArgs[0]).toLowerCase();
    const ledgerWhereSql = sqlToString(whereArgs[1]).toLowerCase();
    expect(primaryWhereSql).toContain("coalesce");
    expect(primaryWhereSql).toContain("claude-sonnet-4");
    expect(ledgerWhereSql).toContain("coalesce");
    expect(ledgerWhereSql).toContain("claude-sonnet-4");
  });
  ...
  test("findUsageLogsWithDetails: model 应按 originalModel 优先语义过滤", async () => {
    vi.resetModules();
    ...
    const { findUsageLogsWithDetails } = await import("@/repository/usage-logs");
    await findUsageLogsWithDetails({ page: 1, pageSize: 1, model: "gpt-4.1" });

    expect(whereArgs.length).toBeGreaterThan(0);
    const whereSql = sqlToString(whereArgs[0]).toLowerCase();
    expect(whereSql).toContain("coalesce");
    expect(whereSql).toContain("gpt-4.1");
  });
  ...
  test("findUsageLogsStats: model 应按 originalModel 优先语义过滤", async () => {
    vi.resetModules();
    ...
    const { findUsageLogsStats } = await import("@/repository/usage-logs");
    await findUsageLogsStats({ model: "gemini-2.5-pro" });

    expect(whereArgs.length).toBeGreaterThan(0);
    const whereSql = sqlToString(whereArgs[0]).toLowerCase();
    expect(whereSql).toContain("coalesce");
    expect(whereSql).toContain("gemini-2.5-pro");
  });
});
