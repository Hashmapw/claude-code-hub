...
describe("ProxyForwarder streaming response guards fallback", () => {
  ...
  test("non-hedge early-error stream guard should fail before flushing and continue with next provider", async () => {
    const provider1 = createProvider(1, {
      name: "early-error-provider",
      firstByteTimeoutStreamingMs: 0,
      maxRetryAttempts: 3,
      rejectStreamingEarlyError: true,
    });
    const provider2 = createProvider(2, {
      name: "healthy-provider",
      firstByteTimeoutStreamingMs: 0,
    ...
    const earlyErrorStream = [
      ...
      "",
      'event: error\ndata: {"type":"error","code":"request_failed","message":"temporary unavailable"}',
      "",
      "",
    ].join("\n");
    ...
    doForward
      .mockResolvedValueOnce(createSseResponse(earlyErrorStream))
      .mockResolvedValueOnce(createSseResponse("data: provider-2-ok\n\n"));
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          errorDetails: expect.objectContaining({
            provider: expect.objectContaining({
              upstreamParsed: expect.objectContaining({
                error: expect.objectContaining({ type: "streaming_early_error_rejected" }),
              }),
            }),
          }),
        }),
