...
describe("proxy env base path canonicalization", () => {
  ...
  it("removes erroneous leading /proxy/3000 before ws base path without external redirect", async () => {
    const { default: proxyHandler } = await import("@/proxy");
    const response = proxyHandler(
      makeRequest("/proxy/3000/ws-a/project-b/user-c/vscode/d/proxy/3000/zh-CN/dashboard")
    );
  ...
  it("recovers missing ws middle path for /proxy/3000/zh-CN routes", async () => {
    const { default: proxyHandler } = await import("@/proxy");
    const response = proxyHandler(makeRequest("/proxy/3000/zh-CN/login"));
  ...
  it("does not let short /proxy/PORT env base downgrade canonical ws path", async () => {
    ...
    const { default: proxyHandler } = await import("@/proxy");
    const response = proxyHandler(makeRequest("/ws-a/project-b/user-c/vscode/d/proxy/3000/login"));
  ...
  it("canonicalizes /proxy/3000/zh-CN to env ws base without duplicate proxy prefix", async () => {
    const { default: proxyHandler } = await import("@/proxy");
    const response = proxyHandler(makeRequest("/proxy/3000/zh-CN"));
  ...
  it("normalizes polluted /proxy/PORT/ws-* request in-process without external redirect", async () => {
    ...
    const response = proxyHandler(
      makeRequest(
        "/proxy/3000/ws-6040202d-b785-4b37-98b0-c68d65dd52ce/project-4c3711a6-74af-4560-b60e-8bae6b39e186/user-6b74da98-40fa-4c4b-8795-ac2449fabd1b/vscode/b26ba028-6945-4ca1-8a38-051600c4fe20/5a897237-15be-4e4d-b75b-771edd984c8e/proxy/3000/zh-CN"
      )
    );
