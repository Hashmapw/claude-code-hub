...
function rewriteGeminiUsageMetadata(
  ...
  if (typeof record.totalTokenCount === "number") {
    ...
    if (nextTotal > 0 && record.totalTokenCount !== nextTotal) {
...
export function adjustUsageLikeObjectInPlace(
  ...
  const visit = (current: unknown): boolean => {
    ...
    const directChanged = rewriteDirectUsageRecord(record, config);
...
function rewriteJsonText(text: string, config: StreamUsageAdjustmentConfig): string | null {
  try {
    const parsed = JSON.parse(text);
    if (!adjustUsageLikeObjectInPlace(parsed, config)) {
      return null;
    }
...
function rewriteStreamLine(lineWithEnding: string, config: StreamUsageAdjustmentConfig): string {
  ...
  if (dataPrefixMatch) {
    ...
    const rewritten = rewriteJsonText(payload, config);
    if (rewritten === null) {
      return lineWithEnding;
    }
  ...
  if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
    const rewritten = rewriteJsonText(trimmed, config);
    if (rewritten === null) {
      return lineWithEnding;
    }
...
export function createKeyStreamUsageAdjustmentTransform(
  ...
  const drainCompleteLines = (controller: TransformStreamDefaultController<Uint8Array>) => {
    while (true) {
      ...
      controller.enqueue(encoder.encode(rewriteStreamLine(line, config)));
    }
  };
  ...
  return new TransformStream<Uint8Array, Uint8Array>({
    ...
    flush(controller) {
      ...
      if (buffer) {
        controller.enqueue(encoder.encode(rewriteStreamLine(buffer, config)));
        buffer = "";
      }
    },
  });
...
export function applyKeyStreamUsageAdjustmentToStream(
  ...
  return stream.pipeThrough(createKeyStreamUsageAdjustmentTransform(decision.config));
}
