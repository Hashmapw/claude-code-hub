...
export async function bufferStreamingHeadForEarlyErrorGuard({
  ...
  const readWithinHeadBudget = async (): Promise<
    { timedOut: true } | { timedOut: false; result: ReadableStreamReadResult<Uint8Array> }
  > => {
    const remainingBudgetMs = EARLY_ERROR_HEAD_TIME_BUDGET_MS - (Date.now() - startTime);
    ...
    try {
      const result = await Promise.race<
        { timedOut: true } | { timedOut: false; result: ReadableStreamReadResult<Uint8Array> }
      >([
        readPromise.then((readResult) => ({ timedOut: false, result: readResult }) as const),
      ...
      if (!timedOut && pendingRead === readPromise) {
        pendingRead = null;
      }
    }
  };
  ...
  try {
    while (true) {
      ...
      const readResult = await readWithinHeadBudget();
      if (readResult.timedOut) {
        // 时间预算用尽但 reader.read() 仍在等待：立即放行已攒头部。pending read 会在
        // 重组后的响应流第一次需要继续读取时复用，避免吞掉后续 chunk。
        break;
      }
  ...
  let firstPendingRead = pendingRead;
  pendingRead = null;

  const readNext = async (): Promise<ReadableStreamReadResult<Uint8Array>> => {
    if (firstPendingRead) {
      const result = await firstPendingRead;
      firstPendingRead = null;
      return result;
    }
    return reader.read();
  };
