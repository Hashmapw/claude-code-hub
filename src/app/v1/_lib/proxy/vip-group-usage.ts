...
export function hasVipProviderGroup(groupTag: string | null | undefined): boolean {
...
export async function maybeSendVipGroupUsageAlert(
  ...
  if (!hasVipProviderGroup(providerGroupTag)) {
    return;
  }

  const user = session.messageContext?.user ?? session.authState?.user ?? null;
