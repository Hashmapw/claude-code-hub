...
export const KEY_SOFT_BLOCK_ERROR_CODES = {
  REDIS_UNAVAILABLE: "KEY_SOFT_BLOCK_REDIS_UNAVAILABLE",
  SAVE_FAILED: "KEY_SOFT_BLOCK_SAVE_FAILED",
} as const;

export type KeySoftBlockErrorCode =
  (typeof KEY_SOFT_BLOCK_ERROR_CODES)[keyof typeof KEY_SOFT_BLOCK_ERROR_CODES];
...
export async function setKeySoftBlockConfig(
...
): Promise<{ ok: true } | { ok: false; errorCode: KeySoftBlockErrorCode }> {
  const redis = getStore();
  if (!redis || redis.status !== "ready") {
    return { ok: false, errorCode: KEY_SOFT_BLOCK_ERROR_CODES.REDIS_UNAVAILABLE };
  }
  ...
  try {
    ...
    return { ok: false, errorCode: KEY_SOFT_BLOCK_ERROR_CODES.SAVE_FAILED };
  }
}
