 ...
 * (e.g., /ws-xxx/.../proxy/4000/), we need to dynamically detect and prepend
 ...
 *
 * Example:
 * - Current URL: https://example.com/ws-xxx/proxy/4000/zh-CN/dashboard
 * - Base path: /ws-xxx/proxy/4000
 * - Locale: zh-CN
 * - Page path: /dashboard
 */
...
function getEnvBasePath(): string {
  return normalizeBasePath(process.env.NEXT_PUBLIC_BASE_PATH);
}

function getProxyBasePathFromEnv(): string {
...
function detectBasePathFromPath(currentPath: string): string {
  ...
  // This handles the case of accessing proxy root like /ws-xxx/.../proxy/4000/
 ...
 *
 * @returns The base path (e.g., "/ws-xxx/proxy/4000") or empty string if none
 */
export function getBasePath(): string {
  if (typeof window === "undefined") {
    return getEnvBasePath() || getProxyBasePathFromEnv();
  }
  ...
  const detectedBasePath = detectBasePathFromPath(currentPath);
  if (detectedBasePath) {
  ...
  const proxyEnvBasePath = getProxyBasePathFromEnv();
  if (proxyEnvBasePath) {
    cachedBasePath = proxyEnvBasePath;
    return cachedBasePath;
  }
 ...
 *
 * @returns The API base path (e.g., "/ws-xxx/proxy/4000/api" or "/api")
 */
 ...
 * @returns The full API URL (e.g., "/ws-xxx/proxy/4000/api/auth/login")
 */
