...
function isSegmentMatch(path: string, segment: string, index: number): boolean {
  ...
  if (index + segment.length > path.length) return false;
...
export function getBasePath(): string {
  ...
  const enabled = isSiiProxySupportEnabled();
  if (cachedBasePath !== null && cachedPathname === rawPathname && cachedEnabled === enabled) {
