...
function shouldPersistSoftBlockConfig(config: {
  enabled?: boolean;
  message?: string | null;
}): boolean {
  return config.enabled === true || normalizeSoftBlockMessage(config.message) !== null;
}
...
export async function addKey(data: {
  ...
  try {
    ...
    if (
      shouldPersistSoftBlockConfig({
        enabled: validatedData.softBlockEnabled,
        message: validatedData.softBlockMessage,
      })
    ) {
      ...
      if (!softBlockResult.ok) {
        return { ok: false, error: softBlockResult.error };
      }
    }
