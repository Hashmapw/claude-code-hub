...
export async function getDistinctModelsForKey(
  ...
  const ledgerConditions = buildKeyLedgerConditions(keyString, { keyString, billingModelSource });
  const messageModelExpr = getBillingModelExpr(billingModelSource);
  const ledgerModelExpr = getLedgerBillingModelExpr(billingModelSource);

  const [messageModels, ledgerModels] = await Promise.all([
    db.execute(
      sql`select distinct ${messageModelExpr} as model
          from ${messageRequest}
          where ${messageRequest.key} = ${keyString}
            and ${messageRequest.deletedAt} is null
            and (${EXCLUDE_WARMUP_CONDITION})
            and ${messageModelExpr} is not null`
    ),
    ledgerConditions
      ? db
          .selectDistinct({ model: ledgerModelExpr })
          .from(usageLedger)
          .where(and(...ledgerConditions, sql`${ledgerModelExpr} is not null`))
      : Promise.resolve([]),
  ]);
...
export async function getUsedModels(
  ...
  const [messageModels, ledgerModels] = await Promise.all([
    db
      .selectDistinct({ model: messageModelExpr })
      .from(messageRequest)
      .where(
        and(
          isNull(messageRequest.deletedAt),
          EXCLUDE_WARMUP_CONDITION,
          sql`${messageModelExpr} IS NOT NULL`
        )
      ),
    db
      .selectDistinct({ model: ledgerModelExpr })
      .from(usageLedger)
      .where(and(LEDGER_BILLING_CONDITION, sql`${ledgerModelExpr} IS NOT NULL`)),
  ]);

  return Array.from(
    new Set(
      [...messageModels, ...ledgerModels]
        .map((row) => row.model)
