...
const PROXY_REVISION = "20260301-ws-hard-guard-v4";
...
function getOriginalPathWithSearch(request: NextRequest): string {
  return REQUEST_META.get(request)?.originalPathWithSearch || `${request.nextUrl.pathname}${request.nextUrl.search || ""}`;
}
...
function createTemporaryRedirect(request: NextRequest, pathname: string): Response {
  let targetPath = normalizeRedirectPath(pathname);

  // Final safety guard:
  // if request is already under workspace path, never emit redirects that prepend
  // /proxy/<port> before /ws-... (this is always a polluted form in notebook routing).
  if (hasWorkspaceSegment(request.nextUrl.pathname)) {
    const pollutedMatch = targetPath.match(/^\/proxy\/\d+(\/ws-[^/]+(?:\/.*|$))/);
    if (pollutedMatch?.[1]) {
      targetPath = pollutedMatch[1];
    }
  }
  ...
  if (queryIndex !== -1) {
    targetPathname = targetPath.slice(0, queryIndex) || "/";
    targetSearch = targetPath.slice(queryIndex);
  }

  const normalizedLocation = normalizeRedirectPath(`${targetPathname}${targetSearch}`);
  const locationUrl = request.nextUrl.clone();
  const normalizedQueryIndex = normalizedLocation.indexOf("?");
  locationUrl.pathname =
    (normalizedQueryIndex === -1
      ? normalizedLocation
      : normalizedLocation.slice(0, normalizedQueryIndex)) || "/";
  locationUrl.search =
    normalizedQueryIndex === -1 ? "" : normalizedLocation.slice(normalizedQueryIndex);
  ...
  return new Response(null, {
    ...
    headers: {
      ...
      "x-cch-proxy-rev": PROXY_REVISION,
    },
  });
}
...
function extractWorkspaceAppPath(pathname: string): string | null {
  ...
  if (SUPPORTED_LOCALES.some((locale) => suffixPath === `/${locale}`)) {
    return null;
  }
...
function maybeCanonicalizePathWithEnvProxyBase(request: NextRequest): Response | null {
  ...
  // Recover requests like /proxy/4000/zh-CN/... when env base is /ws-.../proxy/4000.
...
function reprocessWithCanonicalEnvPath(request: NextRequest, targetPath: string): Response | null {
  ...
  const response = proxyHandler(normalizedRequest);
  response.headers.set("x-cch-proxy-rev", PROXY_REVISION);
  return response;
}
...
function maybeRestoreProxyPrefixFromReferer(request: NextRequest): Response | null {
  ...
  // 2) paths like /proxy/4000/zh-CN/... where ws segment is missing
  ...
  redirectUrl.pathname = `${refererBasePath}${recoverPath}`;
  ...
  if (isDevelopment()) {
    logger.info("Recover proxy base path from referer", {
      ...
      restoredPathname: redirectUrl.pathname,
    });
  }
...
function cloneRequestWithPathname(
  ...
  const originalPathWithSearch =
    existingMeta?.originalPathWithSearch || `${request.nextUrl.pathname}${request.nextUrl.search || ""}`;
  ...
  try {
    ...
    if (currentPathWithSearch) {
      ...
      if (targetPath === normalizedCurrentPath) {
        ...
        passthroughResponse.headers.set("x-cch-proxy-rev", PROXY_REVISION);
        return passthroughResponse;
      }
    }

    const html = buildRelativeRedirectHtml(targetPath);
...
function proxyHandler(request: NextRequest): Response {
  ...
  if (workspaceAppPath && workspaceAppPath !== pathname) {
    const hasLocaleInWorkspaceAppPath = SUPPORTED_LOCALES.some(
      (locale) =>
        workspaceAppPath === `/${locale}` || workspaceAppPath.startsWith(`/${locale}/`)
    );
    const hasAuthCookie = Boolean(request.cookies.get(AUTH_COOKIE_NAME));

    if (hasAuthCookie && hasLocaleInWorkspaceAppPath) {
  ...
  const restoredResponse = maybeRestoreProxyPrefixFromReferer(request);
  if (restoredResponse) {
  ...
  // including prefixed paths like /ws-.../proxy/4000/_next/static/...
  if (
  ...
  if (isPublicPath) {
    return convertToRelativeRedirect(localeResponse, currentPathWithSearch);
  }
