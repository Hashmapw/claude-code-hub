...
(function() {
  ...
  function cleanBasePath(path) {
    ...
    for (var i = 0; i < appRoutes.length; i++) {
      var routeIdx = path.indexOf(appRoutes[i]);
...
${clearAuthCookieScript}  var fullPath = basePath + targetPath;
...
async function proxyHandler(request: NextRequest) {
  ...
  if (!session) {
    ...
    const html = buildRelativeRedirectHtml(fullTargetPath, {
      noScriptTarget: fullTargetPath,
