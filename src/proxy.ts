...
const PUBLIC_STATUS_HEADER = "x-cch-public-status";
...
function readPreferredLocaleFromCookie(request: NextRequest): string | null {
  try {
...
function normalizeLoginFromPath(pathWithoutLocale: string): string {
  let fromPath = pathWithoutLocale || "/dashboard";
  ...
  for (let i = 0; i < SUPPORTED_LOCALES.length; i++) {
    const matchedLocale = SUPPORTED_LOCALES.find(
      (locale) => fromPath === `/${locale}` || fromPath.startsWith(`/${locale}/`)
    );
    if (!matchedLocale) {
      break;
    }

    fromPath = fromPath.slice(matchedLocale.length + 1) || "/";
  }

  return fromPath === "/" ? "/dashboard" : fromPath;
}
...
function cloneRequestWithPathname(
  ...
  if (typeof (normalizedUrl as { clone?: unknown }).clone !== "function") {
    Object.defineProperty(normalizedUrl, "clone", {
      value: () => new URL(normalizedUrl.toString()),
      enumerable: false,
      configurable: true,
      writable: true,
    });
...
function buildRequestHeaders(
  request: NextRequest,
  options?: { publicStatus?: boolean; workspaceBasePath?: string }
): Headers {
  const requestHeaders = new Headers(request.headers);
  requestHeaders.delete(PUBLIC_STATUS_HEADER);

  if (options?.workspaceBasePath) {
    requestHeaders.set(WORKSPACE_BASE_PATH_HEADER, options.workspaceBasePath);
  }

  if (options?.publicStatus) {
    requestHeaders.set(PUBLIC_STATUS_HEADER, "1");
  }

  return requestHeaders;
}

function nextWithRequestHeaders(headers: Headers, response?: Response): Response {
  ...
  if (response) {
    for (const [key, value] of response.headers.entries()) {
      passthroughResponse.headers.set(key, value);
    }
  }
  ...
  try {
    ...
    if (currentPathWithSearch) {
      ...
      if (targetPath === normalizedCurrentPath) {
        const passthroughResponse = requestHeaders
          ? nextWithRequestHeaders(requestHeaders)
          : NextResponse.next();
...
function proxyHandler(request: NextRequest): Response {
  ...
  const sanitizedRequestHeaders = buildRequestHeaders(request);
  ...
  if (workspaceAppPath && workspaceAppPath !== pathname) {
    ...
    const isPublicWorkspacePath = PUBLIC_PATH_PATTERNS.some((pattern) =>
      matchesPublicPath(workspacePathWithoutLocale, pattern)
    );
    const isWorkspacePublicStatusPath =
      hasLocaleInWorkspaceAppPath && matchesPublicPath(workspacePathWithoutLocale, "/status");
    ...
    if ((hasAuthCookie || isPublicWorkspacePath) && hasLocaleInWorkspaceAppPath) {
      ...
      const requestHeaders = buildRequestHeaders(request, {
        workspaceBasePath: workspaceBasePath || undefined,
        publicStatus: isWorkspacePublicStatusPath,
      });
    ...
    if (!hasAuthCookie && !isPublicWorkspacePath && !hasLocaleInWorkspaceAppPath) {
      ...
      searchParams.set("from", normalizeLoginFromPath(workspacePathWithoutLocale));
  ...
  const isLocalePrefixedPublicStatusPath = SUPPORTED_LOCALES.some(
    (locale) => pathname === `/${locale}/status` || pathname.startsWith(`/${locale}/status/`)
  );
  if (isLocalePrefixedPublicStatusPath) {
    const requestHeaders = buildRequestHeaders(request, { publicStatus: true });
    return NextResponse.next({
      request: { headers: requestHeaders },
    });
  }
  ...
  const { locale, isLocaleInPath, pathWithoutLocale } = extractLocaleContext(pathname);
  const redirectLocale = isLocaleInPath ? locale : readPreferredLocaleFromCookie(request) || locale;
  const isExplicitPublicStatusPath = matchesPublicPath(pathname, "/status");

  if (isExplicitPublicStatusPath) {
    return convertToRelativeRedirect(
      localeResponse,
  ...
  if (isPublicPath) {
    if (!localeResponse.headers.get("Location")) {
      return nextWithRequestHeaders(sanitizedRequestHeaders, localeResponse);
    }
    return convertToRelativeRedirect(
      localeResponse,
      sanitizedRequestHeaders,
  ...
  if (!authToken) {
    ...
    const loginPath = resolveTargetPathAgainstCurrentBase(`/${redirectLocale}/login`, pathname);
    const searchParams = new URLSearchParams();
    searchParams.set("from", normalizeLoginFromPath(pathWithoutLocale));
    return createRelativeRedirect(loginPath, searchParams);
  }
