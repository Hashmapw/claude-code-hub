...
function proxyHandler(request: NextRequest) {
  ...
  const appPathname = stripProxyBaseForLoginFrom(pathname);
  ...
  if (
    ...
    appPathname.startsWith(API_PROXY_PATH) ||
    appPathname.startsWith(API_PROXY_BETA_PATH)
  ) {
  ...
  if (!authToken) {
    ...
    const from = normalizeLoginFromPath(appPathname, pathWithoutLocale);
