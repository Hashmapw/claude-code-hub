...
- 新增供应商流式响应防护开关：可拒绝携带 `Content-Length` 的 SSE 响应，或拒绝显式返回 0 usage 的 SSE 响应；命中后按 503 供应商失败处理并继续尝试后续供应商。
- 供应商管理表单、批量编辑、REST API、OpenAPI 类型与五语言文案同步支持上述两个开关。

### 其他

- 补充流式响应防护与供应商 fallback 的单元测试覆盖。
