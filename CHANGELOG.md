...
- Provider「流式首包错误故障转移」开关（`rejectStreamingEarlyError`）：开启后扣住 SSE 流头部探测，在出任何真实内容之前若上游返回错误事件（如 `{"type":"error"}` / `event: error`），按 503 中断该供应商并在决策链里切换到后续供应商（本次请求级故障转移，客户端看不到该错误）；出现真实内容或超过时间（2s）/ 字节（512KB）预算即放行透传。仅作用于 SSE，关闭或非 SSE 时零开销，首包延迟有上界、不发心跳。

### 修复

- 上游「假 200」错误检测现在识别 `{"type":"error","code":"...","message":"..."}` 形状的 SSE 错误事件（OpenAI Responses / Codex：error 是 type 的值而非 error 键、message 可能不含 "error" 关键字）；并在状态推断扫描超过 64KB 时用错误 message 兜底，避免巨型 envelope 把错误顶出扫描窗口导致 503 误回退成 502。未开启首包闸门的供应商也据此把这类错误正确记为 503 失败（计熔断、影响后续路由）。
- 修复 ja/dashboard.json 中残留的全角括号（满足 `ja-dashboard-parentheses` 校验）。
...
- 「对客户端隐藏模型重定向」现在覆盖「上游直接返回不同模型名」的场景，不再仅限配置了 `modelRedirects` 的重定向：只要上游实际返回的模型与请求模型不一致，就把发回客户端（流式 `message_start` / 首个 chunk、非流式 JSON、fake-streaming）的 `model` 改回请求模型；模型本就一致时为零操作（不触发 JSON 重新序列化）。使用记录在真实响应模型后显示 `%`，内部统计与计费仍按上游真实模型记录。
...
- 对客户端隐藏模型重定向：发生 `modelRedirects` 重定向时，将流式 `message_start` / 首个 chunk 与非流式 JSON 响应体里的 `model` 字段改写回客户端请求的原始模型，使客户端始终看到自己请求的模型名；内部统计 / 计费仍记录上游真实模型，使用记录在真实响应模型后以 `%` 标记“已向客户端返回请求模型”。覆盖标准流式、非流式与 fake-streaming 路径。
- Key 流式用量随机倍率（管理员专属）：可按 Key 配置命中概率与四个 token 桶（input / output / cache_read / cache_creation）的改写倍率，命中后改写流式响应用量并用于内部统计与计费；该能力为管理员专属，服务端 `addKey` / `editKey` 强制忽略非管理员对该字段的设置，前端表单亦仅对管理员展示。

### 其他

- 新增 `stream_usage_adjustment` Key 配置列与迁移（0107），并补齐对应的表单校验、鉴权缓存、审计快照与多语言文案。
